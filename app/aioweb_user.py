...
class User:
	def __init__(self, db_path):
		...
		self.db_file = Path.join(db_path, "user_system_202604.db")
		self._init_db()

	def _init_db(self):
		"""初始化 SQLite 資料表"""
		with sqlite3_connect(self.db_file) as conn:
			cursor = conn.cursor()
			# 建立 Users 表
			cursor.execute('''
	...
	async def api_login(self, request):
		...
		try:
			...
			with sqlite3_connect(self.db_file) as conn:
				cursor = conn.cursor()
				if phone:
					cursor.execute("SELECT uid, active FROM users WHERE phone=? AND key=?", (phone, key))
				else:
					cursor.execute("SELECT uid, active FROM users WHERE email=? AND key=?", (email, key))
	...
	async def _set_active(self, request, status):
		try:
			...
			with sqlite3_connect(self.db_file) as conn:
				cursor = conn.cursor()
				# 透過 sid 找 uid
				cursor.execute("SELECT uid FROM sessions WHERE sid=?", (sid,))
				res = cursor.fetchone()
				if not res: return web.json_response({"E": "Invalid SID"}, status=401)
				
				uid = res[0]
				cursor.execute("UPDATE users SET active=? WHERE uid=?", (status, uid))
				conn.commit()
